...
import { mockGameHistory } from "@/mocks";
...
const ProfileScreen: React.FC<ProfileScreenProps> = ({ onBack }) => {
    ...
    const { getPlayerStatsData, getUserData } = useContract();
    ...
    const loadPlayerData = async () => {
        ...
        try {
            ...
            if (statsResponse?.value) {
                ...
                setStats({
                    totalGames: statsData["total-games"]?.value || 0,
                    gamesWon: statsData["games-won"]?.value || 0,
                    currentStreak: statsData["current-streak"]?.value || 0,
                    maxStreak: statsData["max-streak"]?.value || 0,
                    averageAttempts:
                        (statsData["average-attempts"]?.value || 0) / 10,
                });
            } else {
            ...
            if (userData?.value) {
                setUsername(userData.value.username?.value || "Player");
            } else {
    ...
    return (
        <View className="flex-1 bg-darkBg">
            <Header title="Profile" subtitle={username} onBack={onBack} />
